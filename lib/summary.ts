...
export async function buildPeriodSummaries(
  ...
  const [
    ...
    supabase
      .from("projects")
      .select("id, status, price, invoice_status, created_at, updated_at"),
    supabase
      .from("support_cases")
      .select("id, status, hours_spent, updated_at"),
    supabase
  ...
  function periodStats(days: number): PeriodStats {
    ...
    const leadsWon = leads.filter(
      (l) => l.status === "vundet" && l.updated_at >= cutoffStr,
    ).length;
    const newCustomers = customers.filter(
      (c) => c.created_at >= cutoffStr,
    ).length;
    const newProjects = projects.filter(
      (p) => p.created_at >= cutoffStr,
    ).length;
    const projectsCompleted = projects.filter(
      (p) => p.status === "afsluttet" && p.updated_at >= cutoffStr,
    ).length;
    const invoicedAmount = projects
      .filter(
        (p) =>
          ["faktureret", "betalt"].includes(p.invoice_status) &&
          p.updated_at >= cutoffStr,
      )
      .reduce((sum, p) => sum + Number(p.price || 0), 0);

    const resolvedCases = supportCases.filter(
      (s) => s.status === "loest" && s.updated_at >= cutoffStr,
    );
    const supportResolved = resolvedCases.length;
    const supportHours = resolvedCases.reduce(
    ...
    const upcomingRenewals =
      agreements.filter(
        (a) => a.renewal_date >= todayStr && a.renewal_date <= forwardEndStr,
      ).length +
      expenses.filter(
        (e) =>
          e.renewal_date &&
          e.renewal_date >= todayStr &&
          e.renewal_date <= forwardEndStr,
      ).length;
