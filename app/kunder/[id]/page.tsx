  ...
  const sections = [
    {
      title: "Projekter",
      icon: Briefcase,
      items: projects.map((p) => ({ id: p.id, label: p.name, status: p.status, tones: projectStatusTones, labels: projectStatusLabels })),
    ...
    },
    {
      title: "Leads",
      icon: Target,
      items: leads.map((l) => ({ id: l.id, label: l.name, status: l.status, tones: leadStatusTones, labels: leadStatusLabels })),
    ...
    },
    {
      title: "Support",
      icon: LifeBuoy,
      items: supportCases.map((s) => ({ id: s.id, label: s.title, status: s.status, tones: supportStatusTones, labels: supportStatusLabels })),
                    ...
                    <ul className="mt-3 space-y-2">
                      {section.items.map((item) => (
                        <li key={item.id} className="flex items-center justify-between gap-3 text-sm">
                          <Link href={section.href(item.id)} className="truncate text-ink/80 hover:underline">
                            {item.label}
                          </Link>
                          <StatusBadge tone={item.tones[item.status as keyof typeof item.tones]}>
                            {item.labels[item.status as keyof typeof item.labels] ?? item.status}
                          </StatusBadge>
                        </li>
                      ))}
                    </ul>
                  )}
                  <Link href={section.newHref} className="link-muted mt-3 inline-flex items-center gap-1">
