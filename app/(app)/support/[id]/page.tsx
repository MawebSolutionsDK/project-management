...
export default async function SupportsagDetailPage({
  params,
}: {
  params: { id: string };
}) {
        ...
        <form action={updateWithId} className="card space-y-4 p-6">
          <div>
            ...
            <select
              name="customer_id"
              required
              defaultValue={supportCase.customer_id}
              className="input"
            >
              {(customers ?? []).map((c) => (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <select
                name="invoice_status"
                defaultValue={supportCase.invoice_status}
                className="input"
              >
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field
              ...
              name="opened_at"
              type="date"
              defaultValue={supportCase.opened_at ?? ""}
            />
          </div>
          <Field
            label="Lukket dato"
            name="closed_at"
            type="date"
            defaultValue={supportCase.closed_at ?? ""}
          />
          <div>
            ...
            <textarea
              name="notes"
              rows={3}
              defaultValue={supportCase.notes ?? ""}
              className="input"
            />
          </div>
