        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-ink/55">
              Standardpriser til aftaler og projekter — prisen kan altid tilpasses den enkelte kunde.
            </p>
          </div>
              ...
              {(products ?? []).map((p) => (
                <tr key={p.id} className="border-t border-line/70 hover:bg-ink/[0.02]">
                  <td className="px-5 py-3">
                    <Link href={`/produkter/${p.id}`} className="font-medium text-ink hover:underline">
                      {p.name}
                    </Link>
                  </td>
                  <td className="px-5 py-3 text-ink/65">{p.category ?? "–"}</td>
                  <td className="px-5 py-3 text-ink/65">
                    {pricingTypeLabels[p.pricing_type as keyof typeof pricingTypeLabels] ?? p.pricing_type}
                  </td>
                  <td className="px-5 py-3 text-ink/65">{Number(p.default_price).toLocaleString("da-DK")} kr.</td>
                  <td className="px-5 py-3">
                    {p.is_active ? (
                      <span className="rounded-full bg-accent-soft px-2 py-0.5 text-xs font-medium text-accent">Aktiv</span>
                    ) : (
                      <span className="rounded-full bg-ink/[0.06] px-2 py-0.5 text-xs font-medium text-ink/50">Inaktiv</span>
                    )}
                  </td>
                </tr>
              ))}
